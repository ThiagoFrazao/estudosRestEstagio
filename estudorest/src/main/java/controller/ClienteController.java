...
public class ClienteController {
	...
	public void adicionarCliente(Cliente novoCliente, @Suspended AsyncResponse response){
		...
		if(novoCliente.verificarValidade()){			
			if(!clienteDAO.verificarExistenciaCliente(novoCliente.getPrimeiroNome(), novoCliente.getSegundoNome())){
				...
				String uriNovoCliente = gerarUriCliente(novoCliente);
				retorno = Resposta.montarRespostaCriacao(uriNovoCliente);
			}else{
	...
	private String gerarUriCliente(Cliente cliente){
		String uriCliente = info.getBaseUri().toString() + "cliente/find?primeiroNome={nomePrim}&segundoNome={nomeSeg}";
		uriCliente = uriCliente.replace("{nomePrim}", cliente.getPrimeiroNome()).replace("{nomeSeg}", cliente.getSegundoNome());
		return uriCliente;
	}
}
